/*
 *
 * Copyright (c) 2019 Nordic Semiconductor, Inc.
 * All rights reserved.
 *
...
const char clear_all_spinel_counters_syntax[] = "[args]";
...
int tool_cmd_clear_all_spinel_counters(int argc, char *argv[])
{
	int ret = 0;
	int c;
	int timeout = 10 * 1000;
	...
	while (1) {
		...
		case 'h':
			print_arg_list_help(reset_option_list, argv[0],
					    clear_all_spinel_counters_syntax);
			ret = ERRORCODE_HELP;
			goto bail;
...

	{
		...
		message = dbus_message_new_method_call(
		    ...
		    WPANTUND_IF_CMD_CLEAR_ALL_SPINEL_COUNTERS
		    );

		fprintf(stderr, "Clearing all spinel counters.\n");
